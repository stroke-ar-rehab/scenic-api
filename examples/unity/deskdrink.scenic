...
behavior DrinkTrain(obj, init_right, init_left, init_pos):
    ...
    take SpeakAction("Place it on your mouth")
    take PlayVideoAction("third.mov")
    take StopAction()
    while True:
        if ObjectCollide(ego.gameObject.joint_angles.mouth, obj.gameObject.position):
            break
        wait 
...
ego = new Scenicavatar at (0,0,0),
    with behavior DrinkTrain(cup, init_right, init_left, init_pos, target)
